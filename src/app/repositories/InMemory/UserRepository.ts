...
export class UserRepository implements IUserRepository {
  ...
  companies: Company[]
  constructor(users: User[], companyUsers: { userUid: string; companyUid: string }[], companies: Company[]) {
    this.users = users
  ...
  async findCompanies(userUid: string): Promise<Company[]> {
    const companyUsers = this.companyUsers.filter((companyUser) => companyUser.userUid === userUid)
    const companies: Company[] = []
    companyUsers.forEach((companyUser) => {
      const company = this.companies.find((company) => company.uid === companyUser.companyUid)
