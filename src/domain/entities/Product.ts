...
export class Product {
  ...
  validateName(name: string) {
    if (!name || name?.length < 3) throw new AppError("Name must be at least 3 characters")

    if (name.length > 50) throw new AppError("Name must be at most 50 characters")

    if (!/^[a-zA-Z0-9 ]+$/.test(name)) throw new AppError("Name must only contain letters and numbers")
  }

  validatePrice(price: number) {
    if (price < 0) throw new AppError("Price must be greater than 0")
    if (isNaN(price)) throw new AppError("You must provide a valid product price")
  }
  ...
  validateVariants(variants: ProductVariant[]) {
    ...
    variants.map((variant) => {
      variant.properties.map((variantObject) => {
        keyOccurrences[variantObject.key] = (keyOccurrences[variantObject.key] || 0) + 1
      })
    })
