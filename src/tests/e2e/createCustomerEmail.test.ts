...
describe('Create customer email', () => {
  ...
  test('should be possible do create a new customer email', async () => {
    await supertest(app).post(`/companies/${company.body.uid}/customers/${customer.body.uid}/emails`).send({ email: 'any@mail.com' }).set(userToken).expect(201)
  ...
  test('should not be possible to create a new customer email if company uid is invalid', async () => {
    const customerEmail = await apiButfly.post(`/companies/invalid/customers/${customer.body.uid}/emails`).send({ email: 'any@mail.com' }).set(userToken)
  ...
  test('should throw an error if customer uid is invalid', async () => {
    const customerEmail = await supertest(app).post(`/companies/${company.body.uid}/customers/invalid/emails`).send({ email: 'any@mail.com' }).set(userToken)
  ...
  test('should not be possible to store a new customer email if customer company is different from logged user company', async () => {
    ...
    const customerEmail = await apiButfly.post(`/companies/${company.body.uid}/customers/${customer.body.uid}/emails`).send({ email: 'any@mail.com' }).set(altUser)
    expect(customerEmail.body.message).toBe('User does not have access to this company')
  })
})