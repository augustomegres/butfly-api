...
describe("CreateCompany", () => {
  it("should be possible to create a new company", async () => {
    await expect(createCompanyUseCase.execute(data, 1)).resolves.toHaveProperty("uid")
  })

  it("should store address in new company request", async () => {
    const company = await createCompanyUseCase.execute({ ...data, addresses: [address] }, 1)
  ...
  it("should not be possible to create a new company with an invalid name", async () => {
    await expect(createCompanyUseCase.execute({ ...data, name: "" }, 1)).rejects.toThrow("Company name is required")

    await expect(createCompanyUseCase.execute({ ...data, name: "a" }, 1)).rejects.toThrow("Company name must be valid")
  })
