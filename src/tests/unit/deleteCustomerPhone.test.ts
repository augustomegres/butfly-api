...
describe("DeleteCustomerPhone", () => {
  test("should be possible do delete a customer phone", async () => {
    ...
    await expect(deleteCustomerPhone.execute(customerPhone.uid, newCustomer.uid)).resolves.toBeUndefined()
  ...
  test("should throw an error if the customer phone uid is not provided", async () => {
    await expect(deleteCustomerPhone.execute("", "customerUid")).rejects.toThrow("Phone uid must be provided")
  })
  ...
  test("should throw an error if customer not exists", async () => {
    await expect(deleteCustomerPhone.execute("phoneUid", "customerUid")).rejects.toThrow("Customer not exists")
  })

  test("should throw an error if the customer phone not exists", async () => {
    const newCustomer = await createCustomer.execute({ companyUid: "uid", data: { name: "Customer name", phones: [] } })
    await expect(deleteCustomerPhone.execute("phoneUid", newCustomer.uid)).rejects.toThrow("Phone not exists")
  })
})
