...
describe("QueryParams", () => {
  ...
  it("should thrown an error if field is invalid", () => {
    expect(() => queryParams.handle("Invalid_query", ["key"])).toThrow(
      "Operator was not found"
    );
  });

  it("should thrown an error if more than one operator was found", () => {
    expect(() => queryParams.handle("Invalid!=_=query", ["key"])).toThrow(
      "You must provide only one operator"
    );
    expect(() => queryParams.handle("Invalid==query", ["key"])).toThrow(
      "You must provide only one operator"
    );
  });

  it('should thrown an error if parameter is invalid', () => {
    expect(() => queryParams.handle("(key=value;key2=value2)", ["key"])).toThrow(
      "Invalid query param provided, valid fields are: key"
    );
  })
});
