...
describe("CreateUser", () => {
  ...
  it("should not be possible to create an user with an invalid name", async () => {
    await expect(createUserUseCase.execute({ ...validUser, name: "Inv4l1d N4m3" })).rejects.toThrow("Invalid name provided.")
  })

  it("should not be possible to create an user with a incomplete name", async () => {
    await expect(createUserUseCase.execute({ ...validUser, name: "First" })).rejects.toThrow("You must provide last name.")
  })

  it("should not be possible to create an user with an invalid email", async () => {
    await expect(createUserUseCase.execute({ ...validUser, email: "invalid@mail" })).rejects.toThrow("Email is invalid.")
  })

  it("should not be possible to create an user with an invalid password", async () => {
    await expect(createUserUseCase.execute({ ...validUser, password: "1234567" })).rejects.toThrow("Password must have at least 8 characters.")
  })
  ...
  it("should not possible to create an user with an email already registered", async () => {
    ...
    await expect(createUserUseCase.execute(validUser)).rejects.toThrow("Email already registered.")
  })
})
